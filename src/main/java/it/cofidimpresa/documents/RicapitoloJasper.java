...
public class RicapitoloJasper extends CommonDocument {
	private static final Logger logger = Logger.getLogger(RicapitoloJasper.class);
	...
	public String createRicapitolo(Integer idSocio, Integer idFinanziamenti) {
		try {
			super.init(idSocio, idFinanziamenti);
			String fileTemplate = pathTemplate+"RicapitoloA4.jrxml";
	        ...
	        params.put("attivita", socio.getTipologiaMerceologica());
			params.put("garanzia"," GARANZIA DEL ".concat(finanziamentiData.getPercentualeGaranzia()).concat("%"));
			params.put("sommario_uno","DOCUMENTO DI RICONOSCIMENTO");
			params.put("sommario_due","CERTIFICATO DI RESIDENZA STORICO");
			params.put("sommario_tre","VISURA CAMERALE");
			params.put("sommario_quattro","MODELLO UNICO");
			params.put("parte_finanziaria","RICHIESTA € ".concat(Integer.toString(finanziamentiData.getImportoDeliberato())).concat(",00 N.RATE ").concat(finanziamentiData.getRateRichieste()).
					concat(" APPROVATA DAL CONSIGLIO IN DATA ").concat(finanziamentiData.getDataApprovazioneConsiglio()));
	        ...
	        String fileName = pathDocument.concat("Ricapitolo_").concat(socio.getImpresa().concat(".docx"));
