...
public class DomandaJasper extends CommonDocument {
	private static final Logger logger = Logger.getLogger(DomandaJasper.class);
	...
	public String createDomandaBanca(Integer idSocio, Integer idFinanziamenti) {
		try {
			super.init(idSocio, idFinanziamenti);
			String fileTemplate = pathTemplate + "DomandaBancaA4.jrxml";
	        ...
	        params.put("data_Delibera", "Spett.Le Banca di Credito Coop");
	        params.put("sommario_tre", "BCC TERRA DI LAVORO");
	        params.put("indirizzo", "81022 - CASAGIOVE (CE)");
			...
			params.put("sommario_uno","Sul finanziamento saranno addebitati a favore di CONFIDI IMPRESA sul c/c 1963 gli importi come indicato in allegato.");
			params.put("parte_finanziaria","al finanziamento che codesta Banca di Credito Coop BCC Casagiove vorrà  concedere, dell'importo di "
					.concat(Integer.toString(finanziamentiData.getImportoDeliberato()))
					.concat(",00 da rimborsare in n.").concat(finanziamentiData.getRateRichieste()).concat(" mensili."));
	        ...
	        String fileName = pathDocument.concat("DomandaBanca_").concat(socio.getImpresa().concat(".docx"));
	...
	private String setBodyDitta(SociData socio) {
		String result=("CONFIDI IMPRESA in data ".concat(finanziamentiData.getDataApprovazioneConsiglio())
				.concat(" ha deliberato di concedere a ").concat(socio.getImpresa()).concat("C.F: ")
				.concat(socio.getCodiceFiscaleTitolare()).concat(" nato a ").concat(socio.getLuogoDiNascita())
