...
public class Ricapitolo extends CommonDocument {

	public void createRicapitolo(Integer idSocio, Integer idFinanziamenti) {
		try {
			
			super.init(idSocio, idFinanziamenti);

			String fileName = "Ricapitolo".concat(socio.getImpresa().concat(".doc"));
			...
			run5.setBold(false);
			run5.setText("DOCUMENTO DI RICONOSCIMENTO");
			run5.addBreak();
			run5.addBreak();

			run5.setText("CERTIFICATO DI RESIDENZA STORICO");
			run5.addBreak();
			run5.addBreak();

			run5.setText("VISURA CAMERALE");
			run5.addBreak();
			run5.addBreak();
			
			run5.setText("MODELLO UNICO");
			run5.addBreak();	

			XWPFParagraph paragraph9 = document.createParagraph();
			paragraph9.setBorderBottom(Borders.BASIC_BLACK_DASHES);
			paragraph9.setBorderLeft(Borders.BASIC_BLACK_DASHES);
			paragraph9.setBorderRight(Borders.BASIC_BLACK_DASHES);
			paragraph9.setBorderTop(Borders.BASIC_BLACK_DASHES);

			XWPFRun run9 = paragraph9.createRun();
			run9.setBold(true);
			run9.setText("PARTE FINANZIARIA");
			run9.addBreak();
			
			XWPFParagraph paragraph10 = document.createParagraph();
			paragraph10.setBorderBottom(Borders.BASIC_BLACK_DASHES);
			paragraph10.setBorderLeft(Borders.BASIC_BLACK_DASHES);
			paragraph10.setBorderRight(Borders.BASIC_BLACK_DASHES);
			paragraph10.setBorderTop(Borders.BASIC_BLACK_DASHES);

			XWPFRun run10 = paragraph10.createRun();
			run10.setText("RICHIESTA € ".concat(finanziamentiData.getImporto()).concat(",00 N.RATE ").concat(finanziamentiData.getRate()));
			run10.addBreak();
			run10.setText("APPROVATA DAL CONSIGLIO IN DATA ".concat(finanziamentiData.getDataApprovazioneConsiglio()));
			...
			run11.setText("BANCA Banca di Credito Coop BCC Casagiove FILIALE DI Casagiove");
			run11.addBreak();
			run11.addBreak();
			run11.setText("Caserta il");
			run11.addBreak();
